...
def get_reward(control, pos1, ori1, pos2, ori2, torque, vel):
    ...
    if control == 0:
        ...
        r += 2e1 * -del_gamma - np.abs(del_d) / 10
    elif control == 5:
        # r = del_gamma - np.abs(del_f) - np.abs(del_l)
        r += 2e1 * del_gamma - np.abs(del_d) / 10
    return r
...
class Agent_net(nn.Module):
    ...
    '''
    def __init__(self, n_obs: int, n_act: int, dev="cuda", control_size=6) -> None:
        super().__init__()
    ...
    def forward(self, x, control):
        ...
        obs_mod = torch.cat((x, one_hot), dim=1)
        # print(one_hot, obs_mod)
        out = self.net_head(obs_mod)
        # out, self.hidden_cell = self.lstm(out, self.hidden_cell)
        for net in self.net_tail:
            if isinstance(net, Lin_non_Lin):
                residual = net(out)
                out = out + residual
            else:
                out = net(out)
...
class Critic_net(nn.Module):
    ...
    '''
    def __init__(self, n_obs: int, n_act: int, control_size = 6, dev="cuda") -> None:
        super().__init__()
    ...
    def forward(self, obs, act, control):
        one_hot = torch.zeros((obs.shape[0], self.control_size), device=self.dev, dtype=torch.float32)
        for i in range(len(control)):
            one_hot[i, control[i]] = 1
        obs_mod = torch.cat((obs, one_hot), dim=1)
        obs_enc = self.obs_head(obs_mod)
...
class Agent():
    ...
    def move_k_steps(self, s, env, control, ep_len = 1, k=None):
        ...
        r = get_reward(control[0], self.last_pos_n_ori[0], self.last_pos_n_ori[1], pos2, rot2, torque, vel)
        # r *= np.log(4 * ep_len)
        self.curr_ep_r += r
    ...
    def get_batch(self, batch_size=BATCH_SIZE):
        ...
        if len(self.replay_buf) > batch_size - batch_size//2:
            get_batch_from_buff( self.replay_buf_initial, batch_size // 2, control, s, s_next, r, a, is_done)
            get_batch_from_buff( self.replay_buf, batch_size - batch_size // 2, control, s, s_next, r, a, is_done)
        else:
    ...
    def train_iter(self, batch_size=None, rep_steps=None):
        ...
        q_m = self.critic_net(s, a, control)
        # a_next_t = self.action_net_targ(s_next, control)
        a_next = self.action_net(s_next, control)
        _q_t = self.critic_net_targ(s_next, a_next, control)
        _q_t[is_done] = 0.
        ...
        loss_actor = -self.critic_net(s, a_m, control).mean()
        loss_actor.backward()
    ...
    def save_net(self, epoch, path="."):
        ...
        path = f"{path}/model_trotting_{epoch}.pt"
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        agent.epsilon = MAX_EPSILON - (MAX_EPSILON - MIN_EPSILON) * (i / 5_000)
